...
prompt_dir() {
  ...
  $1_prompt_segment "$0" "blue" "white" "$current_path"
}
...
prompt_batt() {
  if [ -n "$(type battery_level_gauge)" ]; then
    local batt_gauge=battery_level_gauge
    "$batt_gauge"
  fi
}
...
build_right_prompt() {
  ...
  if [[ "${#POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS}" == 0 ]]; then
    POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(batt longstatus history time)
  fi
